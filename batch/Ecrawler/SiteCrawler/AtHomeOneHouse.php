...
use Ecrawler\Model\UrlQueue;

class AtHomeOneHouse extends BaseSiteCrawler {
    private $baseUrl = 'http://toushi-athome.jp/ei_42/';
    private $domainUrl = 'http://toushi-athome.jp';
    private $noPrintingImg = 'pic_now_printing.gif';
    protected $siteId = 1;
    private $logger;
    private $itemXPathDic = [
            "name" => '//tr[th[contains(text(), "建物名") and contains(text(), "部屋番号")]]/td[2]/text()',
            ...
            "administrative_expense" => '//tr[th[text()="管理費等"]]/td[2]/text()',
            "repair_financial_reserve" => '//tr[th[text()="修繕積立金"]]/td[1]/text()',
            "tenancy" => '//tr[th[contains(text(),"借地期間")]]/td[2]/text()',
            "premium" => '//tr[th[text()="権利金"]]/td[1]/text()',
            "deposit" => '//tr[th[text()="敷金または保証金"]]/td[2]/text()',
            "maintenance_cost" => '//tr[th[text()="維持費等"]]/td[1]/text()',
            "lump_sum" => '//tr[th[text()="その他一時金"]]/td[2]/text()',
            "annual_planned_income" => '//tr[th[text()="年間予定収入"]]/td[1]/text()',
            "yield" => '//tr[th[text()="利回り"]]/td[2]/text()',
            "layout" => '//tr[th[text()="間取り"]]/td[1]/text()',
            "storey" => '//tr[th[text()="階建 / 階"]]/td[2]/text()',
            "building_area" => '//tr[th[text()="使用部分面積"]]/td[1]/text()',
            "land_area" => '//tr[th[text()="土地面積"]]/td[2]/text()',
            "building_structure" => '//tr[th[text()="建物構造"]]/td[1]/text()',
            "time_old" => '//tr[th[text()="築年月"]]/td[2]/text()',
            "land_right" => '//tr[th[text()="土地権利"]]/td[1]/text()',
            "city_planning" => '//tr[th[text()="都市計画"]]/td[1]/text()',
            "restricted_zone" => '//tr[th[text()="用途地域"]]/td[2]/text()',
            "coverage_ratio" => '//tr[th[text()="建ぺい率"]]/td[1]/text()',
            "floor_area_ratio" => '//tr[th[text()="容積率"]]/td[2]/text()',
            ...
            "private_road_burden_area" => '//tr[th[text()="私道負担面積"]]/td[1]/text()',
            "contact_with_road" => '//tr[th[text()="接道状況"]]/td[2]/text()',
            "classification_of_land_and_category" => '//tr[th[text()="地目"]]/td[1]/text()',
            "geographical_features" => '//tr[th[text()="地勢"]]/td[2]/text()',
            "the_total_number_of_houses" => '//tr[th[text()="総戸数"]]/td[1]/text()',
            "country_law_report" => '//tr[th[text()="国土法届出"]]/td[2]/text()',
            "reform_and_renovation_important_notice" => '//tr[th[text()="リフォーム / リノベーション"]]/td[1]/text()',
            "special_report" => '//tr[th[text()="特記事項"]]/td[1]/text()',
    ...
    public function getUrlList() {
        ...
        $targetSelector = '//ul[@class="floLinkList"]/li/a[starts-with(@href,"/ei_42/1")]/@href'; // 地域別
    ...
    public function getSearchArticleInfo(ArticleInfo $article){
        ...
        if (is_null($articleTypeId)) {
            if (strpos($article->get("article_item"), "アパート") !== false) {
                $articleTypeId = SearchArticle::getArticleTypeId("1棟アパート");
            } else {
                $articleTypeId = SearchArticle::getArticleTypeId("1棟マンション");
            }
        }
